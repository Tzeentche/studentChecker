...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    @Query("SELECT st FROM Student st WHERE st.lastName = :lastName AND st.firstName = :firstName AND st.middleName = :middleName " +
            "AND st.dateOfBirth = :dateOfBirth AND st.passportSeria = :passportSeria AND st.passportNumber = :passportNumber AND " +
